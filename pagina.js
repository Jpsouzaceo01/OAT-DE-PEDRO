async function consultarDDD() {
    ...
    const url = `https://brasilapi.com.br/api/ddd/v1/${ddd}`; // Use backticks for template literals

    try {
        const response = await fetch(url);

        if (!response.ok) {
            throw new Error(`Erro: ${response.status} - ${response.statusText}`); // Use backticks for template literals
        }
